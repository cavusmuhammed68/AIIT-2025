...
# Update the legend to specify "Low Crime" and "High Crime"
legend_html = """
<div style="position: fixed; 
...
Low-Crime (Secure) Clusters: <span style="color:#2ca02c;">&#9679;</span><br>
High-Crime Clusters: <span style="color:#1f77b4;">&#9679;</span><br>
</div>
"""
...
X = data_cleaned[['Year', 'Month', 'LAT', 'LON', 'Premis Cd']]
...
# Shorten the labels for the y-axis
label_mapping = {
    'Vehicle - Stolen': 'Vehicle - Stolen',
    'Battery - Simple Assault': 'Battery - Simple assault',
    'Burglary From Vehicle': 'Burglary from vehicle',
...
# Mapping Crime Occurrences: Point Map
point_map = folium.Map(location=[latitude, longitude], zoom_start=10)

# Add circle markers
for index, row in data_cleaned.iterrows():
    crime_type = row['Crm Cd Desc']
    color = mcolors.to_hex(colors[index % 10])
    folium.CircleMarker(
        location=(row['LAT'], row['LON']),
        radius=1,
        color=color,
...
point_map_save_path = r'C:\Users\cavus\Desktop\Conference_IEEE\Figures\crime_point_map_colored.html'
point_map.save(point_map_save_path)
print(f"Point map has been saved to '{point_map_save_path}'")

# Secure Region Clustering
crime_locations = data_cleaned[['LAT', 'LON']].dropna().copy()
...
# Identify low-crime clusters
crime_density = crime_locations['Cluster'].value_counts().sort_index()
low_crime_clusters = crime_density[crime_density < crime_density.mean()].index

# Visualize Secure Regions
secure_region_map = folium.Map(location=[latitude, longitude], zoom_start=10)

low_crime_color = "#2ca02c"
other_cluster_color = "#1f77b4"
...
<div style="position: fixed; 
            ...
            background-color:white; padding: 10px;">
<b>Cluster Legend</b><br>
Low Crime: <span style="color:#2ca02c;">&#9679;</span><br>
High Crime: <span style="color:#1f77b4;">&#9679;</span><br>
</div>
"""
secure_region_map.get_root().html.add_child(folium.Element(legend_html))
...
plt.gca().invert_yaxis()
plt.savefig(r"C:\Users\cavus\Desktop\Conference_IEEE\Figures\top_10_crime_types", dpi=600, bbox_inches='tight')
plt.show()
...
# Customize plot appearance with switched labels
plt.ylabel('Crime type', fontsize=22)  # Y-label now describes 'Crime type'
plt.xlabel('Number of crimes', fontsize=22)  # X-label now describes 'Number of crimes'
plt.xticks(fontsize=18, rotation=90)  # Rotate x-ticks for better readability
plt.yticks(fontsize=18)
plt.legend(title='Zone', fontsize=16, title_fontsize=18)  # Adjust legend font sizes
...
# Plot top 5 crimes per zone with gridlines as a horizontal bar chart
fig, ax = plt.subplots(figsize=(12, 8))
for area in selected_zones:
    area_data = top_crimes_by_zone[top_crimes_by_zone['AREA NAME'] == area]
    ax.barh(area_data['Short Crm Cd Desc'], area_data['Count'], label=area)  # Use barh for horizontal bars
...
plt.yticks(fontsize=18)
plt.legend(title='Zone', fontsize=16, title_fontsize=18)  # Adjust legend font sizes
plt.grid(axis='x', linestyle='--', alpha=0.7)  # Add gridlines on the x-axis for horizontal bars
plt.tight_layout()

# Save the figure at 600 DPI to the specified path
plt.savefig(r"C:\Users\cavus\Desktop\Conference_IEEE\Figures\top_5_crimes_by_zone_horizontal.png", dpi=600, bbox_inches='tight')
...
# Separate low-crime and high-crime clusters
low_crime_data = crime_locations[crime_locations['Cluster'].isin(low_crime_clusters)]
high_crime_data = crime_locations[~crime_locations['Cluster'].isin(low_crime_clusters)]

# Create a HeatMap
secure_region_heatmap = folium.Map(location=[latitude, longitude], zoom_start=10)

# Add high-crime data with a red gradient
HeatMap(
    data=high_crime_data[['LAT', 'LON']].values,
    radius=10,
    gradient={0.2: 'orange', 0.5: 'red', 1.0: 'darkred'},
    blur=15,
    max_zoom=13
).add_to(secure_region_heatmap)

# Add low-crime data with a green gradient
HeatMap(
    data=low_crime_data[['LAT', 'LON']].values,
    radius=10,
    gradient={0.2: 'lightgreen', 0.5: 'green', 1.0: 'darkgreen'},
    blur=15,
    max_zoom=13
).add_to(secure_region_heatmap)
...
Low Crime: <span style="color:green;">&#9679;</span><br>
High Crime: <span style="color:red;">&#9679;</span><br>
</div>
...
"""
secure_region_heatmap.get_root().html.add_child(folium.Element(legend_html))

# Save the secure region heatmap
secure_heatmap_path = r'C:\Users\cavus\Desktop\Conference_IEEE\Figures\secure_region_heatmap.html'
secure_region_heatmap.save(secure_heatmap_path)
print(f"Secure region heatmap has been saved to '{secure_heatmap_path}'")
